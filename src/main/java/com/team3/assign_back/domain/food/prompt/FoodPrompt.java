...
public class FoodPrompt {
    ...
    public final static String FOOD_LIST = """
            ...
            [작업]
            1. 한식, 중식, 일식, 양식, 분식, 아시안, 패스트푸드, 기타 중 하나에 해당하는 입력값을 받고 그대로 출력
    ...
    public final static String FOOD_ANALYSIS = """
            ...
            3. 290-310자 두 번째 설명문 작성(구체적 재료, 조리법, 식감 특성, 맛과 향, 회식에 어울리는지와 그 이유)
            4. 해당 음식 한국 평균 가격 출력 (단위는 원, 숫자만 출력)
            ...
            - 두 번째 설명문에 회식으로 먹을 외식인지에 대한 이유를 엄격하게 평가해 포함
            - 회식 적합성 평가 기준:
              * 개인 식사형 음식(죽, 일인분 면요리, 개인 도시락 등)은 회식에 부적합(낮은 평가)
              * 여러 사람이 함께 나눠 먹기 좋은 음식(삼겹살, 전골, 탕 등)은 회식에 적합(높은 평가)
              * '공유 가능성'과 '술과의 조화', '회식 분위기 적합성' 측면에서 구체적으로 평가
            - 중복된 표현이나 불필요한 수식어 사용 금지
            - JSON 형식으로만 응답
            """;
}
